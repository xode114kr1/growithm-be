...
githubController.savePendingData = async (req, res, next) => {
  try {
    ...
    await Problem.findOneAndUpdate(
      {
        userId: newPending.userId,
        platform: newPending.platform,
        problemId: newPending.problemId,
      },
      {
        $set: {
          code: newPending.code,
          timestamp: newPending.timestamp,
          state: "pending",
          link: newPending.link,
          memory: newPending.memory,
          time: newPending.time,
        },
        $setOnInsert: {
          title: newPending.title,
          tier: newPending.tier,
          userId: newPending.userId,
          platform: newPending.platform,
          problemId: newPending.problemId,
          categories: newPending.categories,
